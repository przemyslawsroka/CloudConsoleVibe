...
@Component({
  selector: 'app-packet-mirroring-setup-wizard',
  template: `
    <div class="wizard-container">
      <mat-dialog-content class="wizard-content">
        <div class="wizard-header">
          <h2>Packet Mirroring Setup Wizard</h2>
          <p>Let's configure packet mirroring for network monitoring step by step</p>
        </div>
        ...
        <div *ngIf="currentStep === 1" class="step-content">
          ...
          <p>First, let's determine what you want to accomplish with Packet Mirroring:</p>
          ...
          <div class="role-selection">
            <mat-card class="role-card" [class.selected]="selectedRole === 'producer'" (click)="selectRole('producer')">
              <mat-card-content>
                ...
                <h4>Monitoring Service Provider</h4>
                <p>I want to <strong>provide monitoring services</strong> to other organizations</p>
                <ul class="role-features">
                  <li>Deploy monitoring tools (analytics, security, etc.)</li>
                  <li>Make services available via packet mirroring</li>
                  <li>Manage deployment groups and zones</li>
                  <li>Configure ILB frontends for monitoring tools</li>
                </ul>
              </mat-card-content>
            </mat-card>
            ...
            <mat-card class="role-card" [class.selected]="selectedRole === 'consumer'" (click)="selectRole('consumer')">
              <mat-card-content>
                ...
                <p>I want to <strong>use third-party monitoring services</strong> for my traffic</p>
                <ul class="role-features">
                  <li>Mirror my network traffic for analysis</li>
                  <li>Connect to external monitoring services</li>
                  <li>Apply monitoring policies to VPC networks</li>
                  <li>Manage traffic mirroring rules</li>
                </ul>
              </mat-card-content>
            </mat-card>
          </div>
          ...
          <div *ngIf="selectedRole" class="role-explanation">
            ...
            <div class="explanation-content">
              <h4 *ngIf="selectedRole === 'producer'">As a Monitoring Service Provider:</h4>
              <h4 *ngIf="selectedRole === 'consumer'">As a Traffic Owner (Consumer):</h4>
              <p *ngIf="selectedRole === 'producer'">
                You'll create deployment groups and deployments to make your monitoring services available.
                Other organizations can then connect to your services to mirror their traffic for analysis.
              </p>
              <p *ngIf="selectedRole === 'consumer'">
                You'll create endpoint groups that reference external monitoring services, then associate
                them with your VPC networks to enable traffic mirroring for analysis.
              </p>
            </div>
          </div>
        </div>
        ...
        <div *ngIf="currentStep === 2" class="step-content">
          <h3 *ngIf="selectedRole === 'producer'">Step 2: Provider Configuration</h3>
          <h3 *ngIf="selectedRole === 'consumer'">Step 2: Consumer Configuration</h3>

          <!-- Producer Configuration -->
          <div *ngIf="selectedRole === 'producer'">
            <p>Configure your monitoring service deployment:</p>

            <form [formGroup]="producerForm" class="config-form">
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>Deployment Group Name *</mat-label>
                <input matInput formControlName="deploymentGroupName" placeholder="my-monitoring-service">
                <mat-icon matSuffix matTooltip="Logical container for your monitoring service deployments">info</mat-icon>
                <mat-hint>Choose a descriptive name for your monitoring service</mat-hint>
              ...
              <mat-form-field appearance="outline" class="full-width">
                ...
                <textarea matInput formControlName="description" rows="3" 
                  placeholder="Describe your monitoring service and its capabilities"></textarea>
              ...
              <mat-form-field appearance="outline" class="full-width">
                ...
                <mat-hint>VPC network where your monitoring tools are deployed</mat-hint>
              ...
              <div class="deployment-zones">
                ...
                <p>Select zones where you want to deploy monitoring tools:</p>
          ...
          <div *ngIf="selectedRole === 'consumer'">
            <p>Configure traffic mirroring for your networks:</p>

            <form [formGroup]="consumerForm" class="config-form">
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>Endpoint Group Name *</mat-label>
                <input matInput formControlName="endpointGroupName" placeholder="corp-monitoring-endpoints">
                <mat-icon matSuffix matTooltip="Virtual representation of the monitoring service you want to use">info</mat-icon>
                <mat-hint>Choose a name that identifies the monitoring service</mat-hint>
              ...
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>Monitoring Service Provider *</mat-label>
                <mat-select formControlName="deploymentGroup">
                  <mat-option *ngFor="let provider of monitoringProviders" [value]="provider.id">
                    {{ provider.name }} - {{ provider.organization }}
                  </mat-option>
                </mat-select>
                <mat-hint>Choose the monitoring service you want to use</mat-hint>
                <mat-error *ngIf="consumerForm.get('deploymentGroup')?.hasError('required')">
                  Monitoring service provider is required
                </mat-error>
              </mat-form-field>

              <div class="vpc-selection">
                <h4>VPC Networks to Monitor</h4>
                <p>Select which VPC networks should have their traffic mirrored:</p>
        ...
        <div *ngIf="currentStep === 3" class="step-content">
          ...
          <!-- Producer Resources -->
          <div *ngIf="selectedRole === 'producer'">
            <p>Configure monitoring tool deployments:</p>

            <div class="deployments-section">
              <h4>Monitoring Tool Deployments</h4>
                  ...
                  <mat-card-content>
                    <form [formGroup]="getDeploymentForm(i)" class="deployment-form">
                      <mat-form-field appearance="outline">
                        ...
                        <input matInput formControlName="name" [placeholder]="'monitoring-' + zone.id">
                      ...
                      <mat-form-field appearance="outline">
                        ...
                        <mat-hint>Internal Load Balancer that fronts your monitoring tools</mat-hint>
          ...
          <!-- Consumer Resources -->
          <div *ngIf="selectedRole === 'consumer'">
            <p>Configure mirror profiles and associations:</p>

            <div class="mirror-profiles-section">
              <h4>Mirror Profile Configuration</h4>
              <form [formGroup]="mirrorProfileForm" class="profile-form">
                <mat-form-field appearance="outline" class="full-width">
                  <mat-label>Mirror Profile Name *</mat-label>
                  <input matInput formControlName="profileName" placeholder="custom-mirror-profile">
                  <mat-error *ngIf="mirrorProfileForm.get('profileName')?.hasError('required')">
                ...
                <mat-form-field appearance="outline" class="full-width">
                  ...
                  <mat-select formControlName="profileType" [disabled]="true">
                    <mat-option value="CustomMirror">Custom Mirror</mat-option>
                  </mat-select>
                  <mat-hint>Packet Mirroring uses Custom Mirror profiles for third-party services</mat-hint>
                </mat-form-field>
              </form>
                ...
                <div *ngFor="let networkId of selectedNetworks" class="association-item">
                  <mat-card class="association-card">
                    <mat-card-content>
                      ...
                      <p>Traffic from this VPC will be mirrored to the monitoring service</p>
                      <div class="association-details">
                        <span><strong>Zones:</strong> Auto-configured based on monitoring service availability</span>
                        <span><strong>PSC Endpoints:</strong> Will be created automatically for traffic copying</span>
                      </div>
                    </mat-card-content>
                  </mat-card>
                </div>
        ...
        <div *ngIf="currentStep === 4" class="step-content">
          ...
          <p>Please review your Packet Mirroring configuration before proceeding:</p>
            ...
            <mat-card class="review-card">
              ...
              <mat-card-content>
                <div class="review-item">
                  <strong>Role:</strong> {{ selectedRole === 'producer' ? 'Monitoring Service Provider' : 'Traffic Owner (Consumer)' }}
                </div>
                ...
                <div *ngIf="selectedRole === 'consumer'">
                  ...
                  <div class="review-item">
                    <strong>Monitoring Provider:</strong> {{ getProviderName(consumerForm.get('deploymentGroup')?.value) }}
                  </div>
                  <div class="review-item">
                    <strong>Mirror Profile:</strong> {{ mirrorProfileForm.get('profileName')?.value }}
                  </div>
                  <div class="review-item">
            ...
            <div class="next-steps">
              ...
              <div *ngIf="selectedRole === 'producer'" class="steps-list">
                ...
                <ol>
                  ...
                  <li>Your monitoring service will be available for consumers to connect</li>
              ...
              <div *ngIf="selectedRole === 'consumer'" class="steps-list">
                ...
                <ol>
                  ...
                  <li>You can create firewall rules with mirror profile groups</li>
                  <li>Traffic matching those rules will be mirrored for analysis</li>
                  <li>Monitor mirroring status and performance</li>
                </ol>
              </div>
            </div>
          </div>
...
export class PacketMirroringSetupWizardComponent implements OnInit {
  currentStep = 1;
  ...
  monitoringProviders = [
    { id: 'provider-1', name: 'Network Analytics Platform', organization: 'DataFlow Corp' },
    { id: 'provider-2', name: 'Security Monitoring Suite', organization: 'SecureWatch Inc' },
    { id: 'provider-3', name: 'Application Performance Monitor', organization: 'APM Solutions Ltd' }
  ];

  constructor(
    private fb: FormBuilder,
    private dialogRef: MatDialogRef<PacketMirroringSetupWizardComponent>,
    private snackBar: MatSnackBar,
    private packetMirroringService: PacketMirroringService,
  ...
  initializeForms() {
    ...
    this.mirrorProfileForm = this.fb.group({
      profileName: ['', Validators.required],
      profileType: ['CustomMirror']
    });
  }
    ...
    switch (this.currentStep) {
      ...
      case 3:
        if (this.selectedRole === 'producer') {
          ...
          return this.mirrorProfileForm.valid;
        }
      case 4:
        return true;
      default:
  ...
  getForwardingRulesForZone(zoneId: string) {
    ...
    return [
      { id: `ilb-${zoneId}-1`, name: `monitoring-ilb-${zoneId}` },
      { id: `ilb-${zoneId}-2`, name: `analytics-ilb-${zoneId}` }
    ];
  }
  ...
  getProviderName(providerId: string): string {
    const provider = this.monitoringProviders.find(p => p.id === providerId);
  ...
  createResources() {
    ...
    // Simulate API calls
    setTimeout(() => {
      this.snackBar.open('Packet Mirroring resources created successfully!', 'Close', {
        duration: 5000,
  ...
  loadMonitoringProviders() {
    this.packetMirroringService.getAvailableMonitoringProviders().subscribe({
      next: (providers) => {
        this.monitoringProviders = providers;
      },
      error: (error) => {
        console.error('Error loading monitoring providers:', error);
      }
    });
  }
} 